...
files_to_open = ['1.txt', '2.txt', '3.txt', '4.txt']
...
with open(f'{size}.txt', 'r') as file:
    ...
    for row in csv_reader:
        name = row[0]
        print(f"{name}:", ' '.join(row[1:]))

    print("STR list:", header_row[1:])
...
file_num = int(input("Enter number (1-20): "))

if file_num < 5: # small files
    small_dict = {}
    with open(f'small.txt', 'r') as file:
        ...
        with open(str(file_num)+'.txt', 'r') as file:
            lines = file.readlines()
            for line in lines:
                line = line.strip()
            ...
            print(f"\nDna Sequence #{file_num} matches: {n}")
